...
# ================================================================
# 1. START: LOGICA TA PREFERATĂ (READY + GENERATE)
# ================================================================
...
func _ready() -> void:
	...
	# 1. Creăm instanțe GOALE (exact cum voiai)
...
func restock_market(new_day: String) -> void:
	...
	# 1. Stabilim filtrul zilei
	var daily_filter: String = "" # Default: nimic (orice item)

	match new_day:
		"Friday":
			daily_filter = "rare" # Exemplu: doar iteme rare
			print("🟣 Vinerea se vând iteme rare!")
		"Sunday":
			daily_filter = "food"
			print("🟢 Duminică doar food items!")
		_:
			daily_filter = "" # Zile normale (Tuesday, Monday etc.)
			print("⚪ Zi normală, orice item.")

	# 2. Curățăm sloturile vechi
	for c in get_children():
		c.queue_free()

	# 3. Creăm sloturile noi cu filtrul specificat
	var count := rng.randi_range(1, 6)
	for i in range(count):
		# Trimitem filtrul către funcția de creare
		_create_new_slot(daily_filter)
...
func _on_slot_timeout(old_slot: Node) -> void:
	print("DEBUG: Slot expirat. Îl înlocuiesc.")
	# 1. Distrugem slotul vechi (pentru a reseta vizualul provider-ului)
	if is_instance_valid(old_slot):
		old_slot.queue_free()
	
	# 2. Creăm unul singur nou
	_create_new_slot()
...
func _populate_slot_data(inst: Node, slot: Node, filter_crit: String = "") -> bool:
	...
	# ... (restul funcției tale de creare textură, date, preț etc. rămâne la fel) ...
	# (codul de mai jos e doar pentru context, nu trebuie rescris dacă îl ai)
